...
function HomeHeader() {
    ...
    // Get the appropriate home link based on userType
    const getHomeLink = () => {
        if (userType === 'Recruiter') {
            return "/components/profiles/RecruiterDashboard/Dashboard";
        } else if(userType === 'Company') {
    ...
    return (
        <div className="fixed bg-white h-[80px] w-full flex items-center justify-center shadow-md z-[99999999]">
            <div className='h-full w-[70%] flex items-center pl-2 pr-2 justify-between'>
                ...
                <div className="relative border search-bar flex h-[60%] w-[20%] justify-center items-center bg-white rounded-full">
                    <input
                        type="text"
                        placeholder="Search company, jobs, skills"
                        className="bg-white outline-none text-gray-700 w-full rounded-full h-full pl-3"
                        value={searchTerm}
