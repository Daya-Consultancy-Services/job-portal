...
import { getallJob } from '../../operations/userAPI';

function Section4() {
  // Extract categories from job types that will be available in the data
  const categories = [
    "All Jobs",
    "IT",
    "Sales",
  ...
  const { alluserjob } = useSelector((state) => state.user);
  ...
  // Filter jobs based on selected category
  const filteredJobs = !alluserjob ? [] : 
    selectedCategory === "All Jobs" 
      ? alluserjob 
      : alluserjob.filter(job => job.jobType.toUpperCase() === selectedCategory.toUpperCase());
              ...
              {filteredJobs.slice(0, visibleCards).map((job, index) => (
                ...
                >
                  <div className="bg-white h-[90%] rounded-[30px] flex p-3 flex-col">
                    <div className="image&title h-[25%] w-full flex items-center justify-between pl-3 pr-3 border-b-[0.1px] border-gray-300">
                      <div className="img flex items-center gap-3">
                        <span><img src="#" alt="company-logo" className='h-[60px] w-[60px] bg-yellow-100 rounded-full'/></span>
                        <p className='font-semibold text-[25px]'>{job.companyId?.name || "Company Name"}</p>
                      </div>
                      <div className="save-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-400 hover:text-black cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                    ...
                    <div className="card-details p-2">
                      <h2 className="font-bold text-xl mt-2">{job.jobTitle}</h2>
                      <span className="inline-block bg-gray-200 text-gray-700 px-2 py-1 rounded-full text-xs mt-1">{job.jobType}</span>
                      <div className='grid grid-cols-2 mt-2'>
                        <div className="location&type">
                          <p className="text-gray-500 mt-1">{job.jobLocation}</p>
                        </div>
                        <div className="worktime&salary">
                          <p className="text-gray-500 mt-1">{job.salaryRange}</p>
                        </div>
                      </div>
                      <div className="skills mt-2">
                        <p className="text-sm text-gray-600">Skills: {job.skillRequired?.join(", ") || "Not specified"}</p>
                      </div>
                      <div className="card-btn w-full mt-4 h-[50px] ">
                        <button 
                          onClick={handleApply}
