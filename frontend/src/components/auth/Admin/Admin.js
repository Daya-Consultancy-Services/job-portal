...
import { Users, Briefcase, Clock, Filter, Target, TrendingUp, Award, BarChart2, Camera } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import {  getAllAdminDetail, logout, uploadImage } from '../../../operations/adminapi';
...
const Admin = () => {
  ...
  // You might want to fetch admin data on component mount if it's not already there
  useEffect(() => {
    if (token && !admin.id) {
      dispatch(getAllAdminDetail(token));
    }
  }, [ ]);
    ...
    <div className="bg-gray-100 min-h-screen">
            ...
            <div className="relative">
              <button onClick={handleProfileClick} className="flex items-center space-x-2">
                <div className="relative group w-12 h-12 rounded-full overflow-hidden bg-gray-200">
                  <img 
                    src={admin.image || require("../../../assets/default-profile.jpg")} 
                    alt="Admin profile" 
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-60 bg-white rounded-lg shadow-lg py-2 z-50">
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    <p className="text-center font-medium">{admin.name || 'Admin'}</p>
                    <p className="text-center text-sm text-gray-500">{admin.email || 'admin@example.com'}</p>
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-24">
        ...
        <nav className="flex space-x-6 mb-6 overflow-x-auto">
          ...
          <button 
            className={`px-2 py-2 text-sm font-medium transition-colors ${activeTab === 'companies' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            {/* (The rest of your existing dashboard JSX code goes here) */}
