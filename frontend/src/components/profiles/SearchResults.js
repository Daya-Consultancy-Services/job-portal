...
const SearchResults = () => {
  ...
  // Extract jobs array from the search results
  const jobs = Array.isArray(searchResults) ? searchResults : 
               (searchResults?.jobs || []);
  ...
  jobs.forEach(job => {
    if (job.companyId && job.companyId._id) {
      if (!companyMap[job.companyId._id]) {
        companyMap[job.companyId._id] = job.companyId;
        companies.push(job.companyId);
      }
    }
  });
  ...
  const renderCompanyCard = (company) => {
    ...
    const isExpanded = expandedCompanies[companyId] || false;
      ...
      >
        <div className="flex justify-between items-start">
          <div className="flex-grow">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {company.companyfield && Array.isArray(company.companyfield) && company.companyfield.map((field, index) => (
                <span key={index} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">
                  {field}
                </span>
              ))}
                ...
                {companyJobs.map(job => (
                  <div key={job._id} className="bg-gray-50 p-3 rounded border border-gray-200">
                    <div className="flex justify-between">
                      <h5 className="font-medium">{job.jobTitle}</h5>
                      <span className="text-sm text-green-600">
                        {job.salary ? `₹${job.salary.toLocaleString()}` : job.salaryRange ? job.salaryRange : 'Salary not specified'}
                      </span>
                    </div>
                    <p className="text-sm mt-1 text-gray-600">{job.description}</p>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {job.skillRequired && Array.isArray(job.skillRequired) && job.skillRequired.map((skill, index) => (
                        <span key={index} className="bg-green-100 text-green-800 px-2 py-0.5 rounded-full text-xs">
                          {skill}
                        </span>
                      ))}
