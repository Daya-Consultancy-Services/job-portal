...
const ExtraProfile = ({ onSave, isEdit, initialData = null }) => {
    ...
    useEffect(() => {
        if (isEdit && initialData) {
            setFormData({
                ...initialData,
                language: initialData.language || [] // Ensure language is always an array
            });
        }
    }, [isEdit, initialData]);
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (isEdit) {
                await dispatch(updatePersonaldetails(token, formData));
                toast.success("Personal details updated successfully!");
            } else {
                await dispatch(personalDetails(
                    token,
                    formData.gender,
                    formData.dateOfBirth,
                    formData.martialStatus,
    ...
    const handleReset = async () => {
        try {
            await dispatch(deletePersonaldetails(token));
            toast.success("Personal details reset successfully!");
            setFormData({
                gender: "",
                dateOfBirth: "",
